...
For a new protocol or a model on a native adapter, read
[docs/ADAPTER-API.md](docs/ADAPTER-API.md) first. Sony and JBL now run through
`omaphones-device`: protocol-only Python adapters, declarative model records,
and shared transport/state/lifecycle code in `omaphones/`. Add capability or
transport infrastructure to the host when needed; do not copy it into an adapter.
`Model.js`'s backend registry is generated by `tools/build-adapter-registry`.

Use `tools/new-adapter`, `tools/new-model` and `tools/check-adapter` for native
adapters. Generators create inert drafts. Existing pins/captures remain at their
original paths and must not change. The five legacy adapters retain their
existing workflows below until their separate owner-confirmed migration.
Original Sony/JBL bridge files are frozen regression references, not where new
native protocol support belongs. A green replay does not authorize deployment.
